...
import { ShieldCheck, Truck, Zap, Activity, Info, ClipboardList } from 'lucide-react';
    ...
    <main className="min-h-screen bg-white flex flex-col font-sans">
      ...
      <section className="relative pt-32 pb-20 md:pt-48 md:pb-32 overflow-hidden bg-slate-950">
        ...
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
          <motion.div
            className="absolute -top-[10%] -left-[10%] w-[60vw] h-[60vw] max-w-[800px] max-h-[800px] bg-blue-500/20 rounded-full blur-[120px]"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="max-w-2xl">
              <motion.div
                ...
                className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900/50 backdrop-blur-md text-blue-400 text-sm font-semibold tracking-wide mb-6 border border-blue-500/30"
              >
              ...
              >
                Zimno, które <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-blue-600 drop-shadow-[0_0_15px_rgba(59,130,246,0.3)]">
                  napędza przemysł.
                </span>
              </motion.h1>
              ...
              >
                <Link href="/oferta" className="px-8 py-4 bg-blue-600 text-white font-bold rounded-xl shadow-[0_0_20px_rgba(37,99,235,0.4)] hover:bg-blue-500 hover:shadow-[0_0_30px_rgba(37,99,235,0.6)] transition-all flex items-center justify-center gap-2">
                  Zobacz Ofertę <Zap className="w-4 h-4" />
                </Link>
                <Link href="/logowanie" className="px-8 py-4 bg-slate-900/50 backdrop-blur-sm text-white font-medium rounded-xl border border-slate-700 hover:border-blue-500/50 hover:bg-slate-800 transition-all text-center">
                  Portal Klienta
                </Link>
              </motion.div>
            </div>

            <div className="hidden lg:flex relative h-[600px] w-full overflow-hidden justify-center items-start">
              {gasFacts.map((fact, index) => {
                ...
                return (
                  <motion.div
                    key={index}
                    className="absolute w-80 h-80 bg-slate-800/40 backdrop-blur-xl border border-blue-500/40 rounded-full shadow-[0_0_50px_rgba(37,99,235,0.15)] flex flex-col items-center justify-center p-12 text-center"
                    initial={{ y: -350, opacity: 0, scale: 0.8 }}
                    animate={{ y: 650, opacity: [0, 1, 1, 0], scale: [0.9, 1, 1, 0.9] }}
                    transition={{
                      duration: dropDuration,
                      repeat: Infinity,
                      ease: "easeInOut",
                      delay: index * dropDuration,
                      repeatDelay: (totalFacts - 1) * dropDuration
                    }}
                  >
                    <div className="mb-4 w-12 h-12 bg-slate-950 rounded-full flex items-center justify-center border border-blue-500/50 shadow-[0_0_15px_rgba(37,99,235,0.6)] shrink-0">
                      <Info className="w-6 h-6 text-blue-400" />
                    </div>
                    <p className="text-slate-200 text-sm leading-relaxed font-medium">{fact}</p>
                  </motion.div>
                );
              })}
            </div>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-slate-900">Standard <span className="text-blue-600">GAZSTAR</span></h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            ...
            <FeatureCard
              ...
              title="Panel Zamówień B2B"
              desc="Zamów w kilka sekund, monitoruj historię dostaw i powtarzaj poprzednie zamówienia jednym kliknięciem. Bez telefonu, bez maila."
            />
          </div>
        </div>
      </section>
...
function FeatureCard({ icon, title, desc }: { icon: React.ReactNode, title: string, desc: string }) {
  return (
    <div className="p-6 rounded-3xl bg-gray-50 border border-gray-100 hover:bg-white hover:shadow-[0_10px_30px_rgba(37,99,235,0.1)] hover:-translate-y-1 transition-all duration-300">
