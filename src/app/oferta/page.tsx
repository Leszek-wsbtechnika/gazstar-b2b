    ...
    orderItems.forEach(item => {
      if (item.products) {
        addItem({
          ...
          price: item.products.price, // Pobieramy najnowszą cenę z bazy!
          image_url: item.products.image_url,
          quantity: item.quantity
        });
      }
    });
    alert('Produkty dodano do koszyka (ceny zostały zaktualizowane na dzisiejsze)!');
    router.push('/koszyk');
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm sticky top-32">
              ...
              <button onClick={() => supabase.auth.signOut().then(() => window.location.reload())} className="w-full py-3 bg-slate-100 text-slate-600 rounded-xl font-bold text-sm hover:bg-red-50 hover:text-red-600 transition-colors">
                Wyloguj się
              </button>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {orders.length === 0 ? (
              ...
              orders.map((order) => {
                const netTotal = order.order_items.reduce((sum: number, item: any) => sum + (item.price_at_time * item.quantity), 0);
